...
export const restore = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if(!exsitingDocument) throw new Error("Not found");
    if(exsitingDocument.userId !== userId) throw new Error("Unauthorized");
    ...
    if(exsitingDocument.parentDocument) {
      const parent = await ctx.db.get(exsitingDocument.parentDocument);
